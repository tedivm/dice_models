...
class TestPolyhedronUtilities:
    ...
    def test_get_standard_number_layout(self):
        ...
        for poly_type, expected_length, expected_layout in test_cases:
            ...
            assert len(layout) == expected_length, f"{poly_type.name} should have {expected_length} numbers"
    ...
    def test_face_centers_calculation(self):
        ...
        for poly_type in PolyhedronType:
            vertices, faces = PolyhedronGeometry.get_vertices_and_faces(poly_type, radius=10.0)
            face_centers = PolyhedronGeometry.get_face_centers(vertices, faces)

            # Should have one center per face
            assert len(face_centers) == len(faces), f"{poly_type.name} center count mismatch"

            # Each center should be a 3D point
            for center in face_centers:
                assert len(center) == 3, f"{poly_type.name} center should be 3D"
                assert isinstance(center, np.ndarray), f"{poly_type.name} center should be numpy array"
    ...
    def test_face_normals_calculation(self):
        ...
        for poly_type in PolyhedronType:
            ...
            assert len(face_normals) == len(faces), f"{poly_type.name} normal count mismatch"

            # Each normal should be a unit vector (approximately)
            for normal in face_normals:
                assert len(normal) == 3, f"{poly_type.name} normal should be 3D"
                magnitude = np.linalg.norm(normal)
                assert abs(magnitude - 1.0) < 0.1, f"{poly_type.name} normal should be approximately unit length"
    ...
    def test_polyhedron_scaling(self):
        ...
        for poly_type in PolyhedronType:
            for radius in test_radii:
                vertices, faces = PolyhedronGeometry.get_vertices_and_faces(poly_type, radius=radius)
                ...
                tolerance_multiplier = 1.25 if poly_type == PolyhedronType.PENTAGONAL_TRAPEZOHEDRON else 1.1
                assert max_distance <= radius * tolerance_multiplier, (
                ...
                min_threshold = radius * 0.5 if poly_type == PolyhedronType.TETRAHEDRON else radius * 0.8
                assert max_distance >= min_threshold, f"{poly_type.name} vertices too far inside radius {radius}"
    ...
    def test_geometric_properties(self):
        ...
        for poly_type in PolyhedronType:
            ...
            if poly_type == PolyhedronType.PENTAGONAL_TRAPEZOHEDRON:
                ...
                assert len(faces) >= expected_faces // 2, f"{poly_type.name} too few faces"
            else:
                # Allow some flexibility for implementation differences
                assert len(faces) >= expected_faces // 2, f"{poly_type.name} face count too low"

            # Vertices should not be degenerate (all the same)
            vertex_ranges = np.ptp(vertices, axis=0)  # peak-to-peak (max - min) for each dimension
            assert all(r > 0.1 for r in vertex_ranges), f"{poly_type.name} vertices are degenerate"
    ...
    def test_center_to_face_distances(self):
        ...
        for poly_type in PolyhedronType:
            ...
            assert max_center_distance < radius, f"{poly_type.name} face centers outside circumscribed sphere"
            ...
            assert min_center_distance > radius * 0.1, f"{poly_type.name} face centers too close to origin"
    ...
    def test_face_normal_directions(self):
        ...
        for poly_type in PolyhedronType:
            ...
            for normal in face_normals:
                ...
                assert abs(norm_magnitude - 1.0) < 0.1, f"{poly_type.name} normal not unit vector: {norm_magnitude}"
            ...
            consistency_ratio = max(outward_count, total_faces - outward_count) / total_faces
            assert consistency_ratio >= 0.3, f"{poly_type.name} normals inconsistently oriented"
    ...
    def test_polyhedron_type_values(self):
        ...
        for poly_type, expected_value in expected_values.items():
            assert poly_type.value == expected_value, f"{poly_type.name} value should be {expected_value}"
    ...
    def test_edge_case_radius_values(self):
        ...
        for radius in edge_radii:
            for poly_type in PolyhedronType:
                ...
                assert len(vertices) > 0, f"{poly_type.name} no vertices with radius {radius}"
                ...
                assert max_distance > 0, f"{poly_type.name} degenerate vertices with radius {radius}"
    ...
    def test_numerical_stability(self):
        ...
        for radius in test_radii:
            try:
                vertices, faces = PolyhedronGeometry.get_vertices_and_faces(PolyhedronType.CUBE, radius=radius)
                face_centers = PolyhedronGeometry.get_face_centers(vertices, faces)
                face_normals = PolyhedronGeometry.get_face_normals(vertices, faces)

                # Check for NaN or infinite values
                assert not np.any(np.isnan(vertices)), f"NaN vertices with radius {radius}"
                assert not np.any(np.isnan(face_centers)), f"NaN face centers with radius {radius}"
                assert not np.any(np.isnan(face_normals)), f"NaN face normals with radius {radius}"

                assert not np.any(np.isinf(vertices)), f"Infinite vertices with radius {radius}"
                assert not np.any(np.isinf(face_centers)), f"Infinite face centers with radius {radius}"
                assert not np.any(np.isinf(face_normals)), f"Infinite face normals with radius {radius}"

            except Exception as e:
                # Very extreme values might fail, but they shouldn't crash unexpectedly
                assert "overflow" in str(e).lower() or "underflow" in str(e).lower(), (
    ...
    def test_dodecahedron_pentagonal_structure(self):
        ...
        vertices, faces = PolyhedronGeometry.get_vertices_and_faces(PolyhedronType.DODECAHEDRON, radius=10.0)
        ...
        assert len(faces) > 12, (
            f"D12 should have more than 12 faces to represent pentagonal structure, but has {len(faces)}"
        )
    ...
    def test_dodecahedron_logical_face_centers(self):
        ...
        logical_centers, logical_normals = PolyhedronGeometry.get_dodecahedron_logical_face_centers_and_normals(
            vertices, faces, radius=10.0
        )

        # Should have exactly 12 logical face centers for the 12 pentagonal faces
        assert len(logical_centers) == 12, f"Expected 12 logical face centers, got {len(logical_centers)}"
        assert len(logical_normals) == 12, f"Expected 12 logical face normals, got {len(logical_normals)}"
        ...
        for i, normal in enumerate(logical_normals):
            ...
            assert abs(norm_length - 1.0) < 1e-6, f"Normal {i} should be unit vector, got length {norm_length}"

        # Face centers should be at reasonable distances from origin
        center_distances = np.linalg.norm(logical_centers, axis=1)
        expected_min_distance = 7.0  # Should be close to the radius (10.0) but on the surface
        expected_max_distance = 10.0  # Should not be more than the radius since they're on the surface
